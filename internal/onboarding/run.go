...
// Run signs a user in with their browser and takes them through to a saved
// config in the full-screen UI.
...
// RunHeadless signs a user in with their browser and saves the resulting
// config using plain-text prompts and status output.
func RunHeadless(ctx context.Context, opts Options) (config.Config, error) {
	...
	if r.Group != nil {
		_, _ = fmt.Fprintf(os.Stderr,
			"Created API key %q in group %q and saved it to %s.\nManage it at %s.\n",
			r.KeyName, r.Group.Name, config.DisplayPath(), APIKeysURL)
	} else {
		_, _ = fmt.Fprintf(os.Stderr,
			"Created API key %q in your personal API keys and saved it to %s.\nManage it at %s.\n",
			r.KeyName, config.DisplayPath(), APIKeysURL)
	}
